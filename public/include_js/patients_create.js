...
function resetPatientFormToCreateMode() {
    document.getElementById('patient-modal-title').textContent = LanguageManager.trans('patient.add_new_patient');
...
function setPatientFormToEditMode(patient) {
    document.getElementById('patient-modal-title').textContent = LanguageManager.trans('patient.edit_patient');
...
function updateAgeDisplay(birthday) {
    ...
    if (age !== null) {
        ageSpan.textContent = age + ' ' + LanguageManager.trans('common.years_old');
...
function initEmailValidation() {
    ...
    emailInput.addEventListener('blur', function() {
        ...
        if (email && !isValidEmail(email)) {
            ...
            if (validationDiv) {
                validationDiv.textContent = LanguageManager.trans('common.invalid_email');
...
function addKinRelationRow(data) {
    ...
    row.innerHTML =
        ...
        '<div class="col-md-3">' +
            '<select name="kin_relations[' + idx + '][relationship]" class="form-control">' +
                '<option value="">' + LanguageManager.trans('common.please_select') + '</option>' +
                '<option value="spouse">' + LanguageManager.trans('members.relationship_spouse') + '</option>' +
                '<option value="child">' + LanguageManager.trans('members.relationship_child') + '</option>' +
                '<option value="parent">' + LanguageManager.trans('members.relationship_parent') + '</option>' +
                '<option value="other">' + LanguageManager.trans('members.relationship_other') + '</option>' +
            '</select>' +
        '</div>' +
        '<div class="col-md-3">' +
            '<input type="text" name="kin_relations[' + idx + '][notes]" class="form-control" placeholder="' + LanguageManager.trans('patient.notes') + '">' +
        '</div>' +
        '<div class="col-md-1">' +
            '<button type="button" class="btn btn-sm btn-danger" onclick="removeKinRelationRow(' + idx + ')" title="' + LanguageManager.trans('common.delete') + '">' +
    ...
    select.select2({
        language: window.PatientsCreateConfig.locale,
        placeholder: LanguageManager.trans('patient.kin_search_placeholder'),
        allowClear: true,
...
function initReferredBySelect() {
    $('#referred_by').select2({
        language: window.PatientsCreateConfig.locale,
        placeholder: LanguageManager.trans('patient.referred_by_placeholder'),
        allowClear: true,
        minimumInputLength: 1,
        ajax: {
            url: '/search-patient',
