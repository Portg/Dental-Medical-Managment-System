...
function saveTreatmentPlan() {
    ...
    $.ajax({
        ...
        type: method,
        data: {
            _token: window.TreatmentPlansConfig.csrfToken,
...
function deleteTreatmentPlan(id) {
    swal({
        ...
        $.ajax({
            url: '/treatment-plans/' + id,
            type: 'DELETE',
            data: {_token: window.TreatmentPlansConfig.csrfToken},
            success: function(response) {
                if (response.status) {
                    swal(LanguageManager.trans('common.deleted'), response.message, "success");
                    dataTable.ajax.reload();
                } else {
                    swal(LanguageManager.trans('common.error'), response.message, "error");
                }
            },
            error: function() {
