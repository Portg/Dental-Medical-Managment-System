...
$(function () {
    ...
    dataTable = $('#quotations-table').DataTable({
        ...
        ajax: {
            url: window.QuotationsConfig.indexUrl,
            data: function (d) {
...
//filter patients
$('#patient').select2({
    language: window.QuotationsConfig.locale,
    placeholder: LanguageManager.trans('common.choose_patient'),
...
$("#addQuotationItem").click(function () {
    ...
    $("#QuotationItemsTable").append(
        ...
        '</td>' +
        '<td>' +
        '<input type="text" name="addmore[' + currentIndex + '][tooth_no]" placeholder="' + LanguageManager.trans('common.enter_tooth_no') + '" class="form-control"/>' +
        '</td>' +
        '<td>' +
        '<input type="number" onkeyup="QTYKeyChange(' + currentIndex + ')" id="procedure_qty' + currentIndex + '" name="addmore[' + currentIndex + '][qty]" placeholder="' + LanguageManager.trans('common.enter_qty') + '" class="form-control"/>' +
        '</td>' +
        '<td>' +
        '<input type="number" onkeyup="PriceKeyChange(' + currentIndex + ')" id="procedure_price' + currentIndex + '" name="addmore[' + currentIndex + '][price]" placeholder="' + LanguageManager.trans('common.enter_price') + '" class="form-control"/>' +
        '</td>' +
        '<td>' +
        '<input type="text" readonly id="total_amount' + currentIndex + '" class="form-control"/>' +
        '</td>' +
        '<td>' +
        '<button type="button" class="btn btn-danger remove-tr">' + LanguageManager.trans('common.remove') + '</button>' +
        '</td>' +
        '</tr>'
    );
...
function save_quotation() {
    ...
    $('#btnSave').text(LanguageManager.trans('messages.processing'));
    $.ajax({
        type: 'POST',
        ...
        error: function (request, status, error) {
            ...
            $('#quotation-modal').modal('show');
            $('#btnSave').attr('disabled', false);
            $('#btnSave').text(LanguageManager.trans('quotations.generate_quotation'));
...
function shareQuotationView(quotation_id) {
    ...
    $('#btn-share').text(LanguageManager.trans('quotations.share_quotation'));
    $.ajax({
        type: 'GET',
...
function sendQuotation() {
    ...
    $('#btn-share').text(LanguageManager.trans('messages.processing'));
    $.ajax({
        type: 'POST',
