$(function () {
    dataTable = $('#leave-requests_table').DataTable({
        ...
        ajax: {
            url: window.LeaveRequestsConfig.urls.leaveRequests,
            data: function (d) {
            }
        },
        columns: [
...
function loadLeaveTypes() {
    $.ajax({
        url: window.LeaveRequestsConfig.urls.getAllLeaveTypes,
        dataType: 'json',
        success: function (data) {
            $('#leave_type_id').select2({
                language: window.LeaveRequestsConfig.locale,
                placeholder: LanguageManager.trans('leaves.choose_leave_type'),
                data: data,
                allowClear: true
            });
        },
        error: function () {
            console.error(LanguageManager.trans('leaves.failed_to_load_leave_types'));
        }
    });
}
...
function editRecord(id) {
    ...
    $.ajax({
        type: 'get',
        url: window.LeaveRequestsConfig.urls.leaveRequests + '/' + id + '/edit',
        success: function (data) {
            $('#id').val(id);
...
function update_record() {
    ...
    $.ajax({
        type: 'PUT',
        data: $('#leave-requests-form').serialize(),
        url: window.LeaveRequestsConfig.urls.leaveRequests + '/' + $('#id').val(),
        success: function (data) {
            $('#leave-requests-modal').modal('hide');
            $.LoadingOverlay("hide");
            if (data.status) {
...
function deleteRecord(id) {
    swal({
        ...
        text: LanguageManager.trans('leaves.confirm_delete_request'),
        type: "warning",
        ...
        $.ajax({
            ...
            url: window.LeaveRequestsConfig.urls.leaveRequests + '/' + id,
            success: function (data) {
                if (data.status) {
                    alert_dialog(data.message, "success");
                } else {
