$(function () {
    dataTable = $('#commission_table').DataTable({
        ...
        ajax: {
            url: window.CommissionRulesConfig.urls.commissionRules
        },
        dom: 'rtip',
        columns: [
...
function editRecord(id) {
    ...
    $.ajax({
        type: 'get',
        url: window.CommissionRulesConfig.urls.commissionRules + '/' + id + '/edit',
        success: function (data) {
            $('#id').val(id);
...
function update_record() {
    ...
    $.ajax({
        type: 'PUT',
        data: $('#commission-form').serialize(),
        url: window.CommissionRulesConfig.urls.commissionRules + '/' + $('#id').val(),
        success: function (data) {
            $('#commission-modal').modal('hide');
            if (data.status) {
                alert_dialog(data.message, "success");
...
function deleteRecord(id) {
    swal({
        ...
        text: LanguageManager.trans('commission_rules.delete_confirm'),
        type: "warning",
        ...
        $.ajax({
            ...
            url: window.CommissionRulesConfig.urls.commissionRules + '/' + id,
            success: function (data) {
                if (data.status) {
                    alert_dialog(data.message, "success");
                } else {
