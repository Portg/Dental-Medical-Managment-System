@extends('layouts.app')
@section('head')
    <title>{{ __('invoices.coupons') }} | {{ config('app.name') }}</title>
@endsection
            ...
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        ...
                        <span class="caption-subject bold uppercase">{{ __('invoices.coupons') }}</span>
                    </div>
                    <div class="actions">
                        <a href="#" class="btn btn-circle btn-success" data-toggle="modal" data-target="#couponModal" onclick="resetForm()">
                            <i class="fa fa-plus"></i> {{ __('invoices.new_coupon') }}
                        </a>
                    </div>
                </div>
                <div class="portlet-body">
                    <table class="table table-striped table-bordered table-hover" id="coupons-table">
                        <thead>
                            <tr>
                                <th>#</th>
...
<script>
    ...
    function resetForm() {
        isEdit = false;
        $('#modalTitle').text('{{ __('invoices.new_coupon') }}');
        $('#couponForm')[0].reset();
        $('#coupon_id').val('');
    ...
    function editCoupon(id) {
        isEdit = true;
        $('#modalTitle').text('{{ __('invoices.edit_coupon') }}');
        $('.alert-danger').hide();
