...
#### 检查 5: Anti-Goals 合规
```
参照 ai-dev-template/ai-spec/domain/intent.md，逐条检查：
→ 涉及金额？检查 AG-001~AG-006（折扣审批、退款、bcmath、优先级）
→ 涉及状态？检查 state-machines.yaml 中对应状态机的合法路径
→ 涉及病历？检查 AG-007~AG-009（锁定、Amendment、版本号）
→ 涉及查询？检查 AG-019（软删除）
→ 涉及日志？检查 AG-015（NIN 不入日志）
→ 涉及阈值？检查 AG-020（不硬编码，引用 rule-engine.md）
```

#### 检查 6: 状态流转合规
```
参照 ai-dev-template/ai-spec/domain/state-machines.yaml：
→ 新增/修改的状态流转是否在 transitions 白名单中？
→ 是否会产生新的非法跳跃路径？
→ 状态同步规则（如 WaitingQueue → Appointment）是否正确处理？
```
...
| 5 | Anti-Goals | ✅ | 无违反 intent.md 中的 AG-xxx 条目 |
| 6 | 状态流转 | ✅ | 符合 state-machines.yaml 白名单 |
